...
def main():
    ...
    while True:
        ...
        # Detectar si es una acción JSON (puede estar embebido en texto)
        import re
        json_str = None
        if '"accion"' in respuesta:
            match = re.search(r'(\{.*"accion".*\})', respuesta, re.DOTALL)
            if match:
                json_str = match.group(1)

        if json_str:
            accion_data = _procesar_accion(json_str, estado_pipeline)
            if accion_data:
